...
ensure_supervised_client(ClientId, Hosts, Config) ->
  ?LOG(warning, " ensure supervised client... ClientId: ~p, Hosts: ~p, Config: ~p", [ClientId, Hosts, Config]),
...
stop_and_delete_supervised_client(ClientId) ->
  ?LOG(warning, "stop and delete supervised client... ClientId: ~p", [ClientId]),
...
start_producers(Client, Topic, ProducerCfg) when is_pid(Client) ->
  ?LOG(warning, "start producers... Client: ~p, Topic: ~p, ProducerCfg: ~p", [Client, Topic, ProducerCfg]),
...
stop_producers(Producers) ->
  ?LOG(warning, "stop producers... Produces: ~p", [Producers]),
...
ensure_supervised_producers(ClientId, Topic, ProducerCfg) ->
  ?LOG(warning, "ensure supervised producers... ClientId: ~p, Topic: ~p, ProducerCfg: ~p", [ClientId, Topic, ProducerCfg]),
