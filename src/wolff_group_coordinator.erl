...
%%%
%%% @end
%%% Created : 12. 5月 2020 上午10:18
%%%-------------------------------------------------------------------
-module(wolff_group_coordinator).
-author("zxb").
...
-logger_header("[wolff group coordinator]").

-import(wolff_utils, [kf/2, kf/3]).
...
-define(PARTITION_ASSIGNMENT_STRATEGY_ROUNDROBIN, roundrobin_v2). % default
...
-define(LO_CMD_STABILIZE(AttemptCount, Reason), {lo_cmd_stabilize, AttemptCount, Reason}).

-define(INITIAL_MEMBER_ID, <<>>).

-type protocol_name() :: string().
-type wolff_offset_commit_strategy() :: commit_to_kafka_v2 % default
| consumer_managed.

-type partition_assignment_strategy() :: wolff_offset_commit_strategy().
-type config() :: wolff:config().
-type ts() :: erlang:timestamp().
-type member() :: wolff:group_member().
-type offset_commit_policy() :: wolff_offset_commit_strategy().
...
-record(state, {
  ...
  generationId = 0 :: wolff:group_generation_id(),
  ...
  %% We can not use a payload connection managed by wolff_client
  %% because connections in wolff_client are shared resources,
  ...
  %% 'consumer_managed'
  acked_offsets = [] :: [{{wolff:topic(), wolff:partition()}, wolff:offset()}],
...
%%      heartbeat response received in the past N seconds.</li>
%%
...
%%           assignment re-balance or group coordinator switchover etc.</li>
%%
...
%%            (e.g. wolff_group_subscriber.erl) is responsible for
...
%%  </li>
%% </ul>
-spec start_link(wolff:client_id(), wolff:group_id(), [wolff:topic()],
    config(), module(), pid()) -> {ok, pid()} | {error, any()}.
start_link(ClientId, GroupId, Topics, Config, CbModule, MemberPid) ->
  ?LOG(info, "start_link...~n ClientId:~p~n GroupId:~p~n Topics:~p~n Config:~p~n CbModule:~p~n MemberPid:~p~n", [ClientId, GroupId, Topics, Config, CbModule, MemberPid]),
  Args = {ClientId, GroupId, Topics, Config, CbModule, MemberPid},
  gen_server:start_link(?MODULE, Args, []).

%% @doc For group member to call to acknowledge a consumed message offset.
-spec ack(pid(), wolff:group_generation_id(), wolff:topic(), wolff:partition(), wolff:offset()) -> ok.
ack(Pid, GenerationId, Topic, Partition, Offset) ->
  ?LOG(info, "ack...~n Pid:~p~n GenerationId:~p~n Topic:~p~n Partition:~p~n Offset:~p~n", [Pid, GenerationId, Topic, Partition, Offset]),
...
-spec commit_offsets(pid(), [{{wolff:topic(), wolff:partition()}, wolff:offset()}]) -> ok | {error, any()}.
commit_offsets(CoordinatorPid, Offsets) ->
  ?LOG(info, "commit_offsets...~n CoordinatorPid:~p~n Offsets:~p~n", [CoordinatorPid, Offsets]),
  NewOffsets = lists:ukeysort(1, Offsets),
  try
    gen_server:call(CoordinatorPid, {commit_offsets, NewOffsets}, 5000)
  catch
    exit : {timeout, _} ->
      {error, timeout}
  end.

%% @doc Update the list of topics the wolff_group_coordinator follow which
%% triggers a join group re-balance
-spec update_topics(pid(), [wolff:topic()]) -> ok.
update_topics(CoordinatorPid, Topics) ->
  ?LOG(info, "update_topics...~n CoordinatorPid:~p~n Topics:~p~n", [CoordinatorPid, Topics]),
  gen_server:cast(CoordinatorPid, {update_topics, Topics}).

%%%_* gen_server callbacks =====================================================

init({ClientId, GroupId, Topics, Config, CbModule, MemberPid}) ->
  ?LOG(info, "init..."),
  erlang:process_flag(trap_exit, true),

  Cfg = fun(Name, Default) -> maps:get(Name, Config, Default) end,
  PaStrategy = Cfg(partition_assignment_strategy, ?PARTITION_ASSIGNMENT_STRATEGY_ROUNDROBIN),
  SessionTimeout = Cfg(session_timeout_seconds, ?SESSION_TIMEOUT_SECONDS),
  HbRate = Cfg(heartbeat_rate_seconds, ?HEARTBEAT_RATE_SECONDS),
  MaxRejoinAttempts = Cfg(max_rejoin_attempts, ?MAX_REJOIN_ATTEMPTS),
  RejoinDelay = Cfg(rejoin_delay_seconds, ?REJOIN_DELAY_SECONDS),
  OffsetRetention = Cfg(offset_retention_seconds, ?undef),
  OffsetCommitPolicy = Cfg(offset_commit_policy, ?OFFSET_COMMIT_POLICY),
  OffsetCommitInterval = Cfg(offset_commit_interval_seconds, ?OFFSET_COMMIT_INTERVAL_SECONDS),
  ProtocolName = Cfg(protocol_name, PaStrategy),

  self() ! ?LO_CMD_STABILIZE(0, ?undef),
  start_heartbeat_timer(HbRate),
  ...
  {ok, #state{
    ...
    heartbeat_rate_seconds = HbRate,
    max_rejoin_attempts = MaxRejoinAttempts,
    rejoin_delay_seconds = RejoinDelay,
...
handle_info(?LO_CMD_COMMIT_OFFSETS, #state{is_in_group = true} = State) ->
  ...
  {ok, NewState} =
    try
      ...
      ?LOG(warning, "handle_info<<?LO_CMD_COMMIT_OFFSETS>>...~n throw Reason:~p~n", [Reason]),
      stabilize(State, 0, Reason)
    end,
  {noreply, NewState};
handle_info(?LO_CMD_STABILIZE(N, _Reason), #state{max_rejoin_attempts = Max} = State)
  when N >= Max ->
  ?LOG(warning, "handle_info<<?LO_CMD_STABILIZE>>...~n N:~p~n _Reason:~p~n State:~p~n", [N, _Reason, State]),
  {stop, max_rejoin_attempts, State};
handle_info(?LO_CMD_STABILIZE(N, Reason), State) ->
  ?LOG(warning, "handle_info<<?LO_CMD_STABILIZE>>...~n N:~p~n Reason:~p~n State:~p~n", [N, Reason, State]),
  {ok, NewState} = stabilize(State, N, ?undef),
  {noreply, NewState};
handle_info({'EXIT', Pid, Reason}, #state{connection = Pid} = State) ->
  ?LOG(warning, "handle_info<<EXIT 1>>...~n Pid:~p~n Reason:~p~n State:~p~n", [Pid, Reason, State]),
...
handle_info({'EXIT', Pid, Reason}, #state{member_pid = Pid} = State) ->
  ?LOG(warning, "handle_info<<EXIT 2>>...~n Pid:~p~n Reason:~p~n State:~p~n", [Pid, Reason, State]),
  case Reason of
...
handle_info(?LO_CMD_SEND_HB, #state{hb_ref = HbRef, heartbeat_rate_seconds = HbRate,
  ...
  case HbRef of
    ...
    {_Ref, SentTime} ->
      ...
      case Elapsed < SessionTimeout * 1000000 of
        true ->
...
handle_info({msg, _Pid, #kpro_rsp{api = heartbeat, ref = HbRef, msg = Body}},
  ...
  case ?IS_ERROR(EC) of
    true ->
      ?LOG(warning, "handle_info<<msg>>...~n ErrorMsg:~p~n State0:~p~n", [Body, State0]),
...
handle_info(_Info, #state{} = State) ->
  ?LOG(info, "handle_info<<no handle>>...~n _Info:~p~n State:~p~n", [_Info, State]),
  [noreply, State].

handle_call({commit_offsets, ExtraOffsets}, From, State) ->
  ?LOG(info, "handle_call<<commit_offsets>>...~n ExtraOffsets:~p~n State:~p~n", [ExtraOffsets, State]),
  try
    ...
    ?LOG(warning, "handle_call<<commit_offsets>>...~n throw Reason:~p~n ", [Reason]),
...
handle_cast({update_topics, Topics}, State) ->
  ?LOG(info, "handle_cast<<update_topics>>...~n Topics:~p~n State:~p~n", [Topics, State]),
...
code_change(_OldVsn, State, _Extra) ->
  ?LOG(info, "code_change...~n _OldVsn:~p~n State:~p~n _Extra:~p~n", [_OldVsn, State, _Extra]),
  {ok, State}.

terminate(Reason, #state{connection = Connection, groupId = GroupId, memberId = MemberId} = State) ->
  ...
  Request = kpro:make_request(leave_group, 0, Params),
  try
...
stabilize(#state{rejoin_delay_seconds = RejoinDelaySeconds,
  ...
  member_pid = MemberPid,
  offset_commit_timer = Timer} = State0, AttemptNum, Reason) ->
  is_reference(Timer) andalso erlang:cancel_timer(Timer),
  Reason =/= ?undef andalso ?LOG(warning, "re-joining group, reason:~p~n", [Reason]),

  %% 1. unsubscribe all currently assigned partitions
  ok = MemberModule:assignments_revoked(MemberPid),

  %% 2. some wolff_group_member implementations may wait for messages
  %%    to finish processing when assignments_revoked is called.
  %%    The acknowledgments of those messages would then be sitting
  ...
  %% 3. try to commit current offsets before re-joining the group.
  ...
  State2 = case AttemptNum =:= 0 andalso Reason =/= ?illegal_generation of
             true ->
  ...
  RetryFun =
    fun(StateIn, ReasonIn) ->
      ?LOG(error, "failed to join group reason: ~p", [ReasonIn]),
      case AttemptNum =:= 0 of
        true ->
          %% do not delay before the first retry
          self() ! ?LO_CMD_STABILIZE(AttemptNum + 1, ReasonIn);
        false ->
          erlang:send_after(timer:seconds(RejoinDelaySeconds), self(), ?LO_CMD_STABILIZE(AttemptNum + 1, ReasonIn))
      end,
      {ok, StateIn}
    end,
...
discover_coordinator(#state{client_id = ClientId, connection = Conn, groupId = GroupId} = State) ->
  ...
  case is_already_connected(State, Endpoint) of
    true ->
      {ok, State};
    false ->
      is_pid(Conn) andalso kpro:close_connection(Conn),
      ConnClientId = make_group_connection_client_id(),
      NewConnConfig = ConnConfig#{client_id => ConnClientId},
      Connection = ?ESCALATE(kpro:connect(Endpoint, NewConnConfig)),
...
join_group(#state{groupId = GroupId, memberId = MemberId, topics = Topics,
  ...
  Request = wolff_kafka_request:join_group(Conn, KafkaParams),
  %% send join group request and wait for response
  %% as long as the session timeout config
  ?LOG(info, "join_group before...~n"),
  Response = send_sync(Conn, Request, SessionTimeout),
  ?LOG(info, "join_group after...~n Response: ~p~n", [Response]),

  GenerationId = kf(generation_id, Response),
  LeaderId = kf(leader_id, Response),
  NewMemberId = kf(member_id, Response),
  Members0 = kf(members, Response),
  Members1 = translate_members(Members0),
  Members = ensure_leader_at_hd(LeaderId, Members1),
  ?LOG(info, " LeaderId:~p, NewMemberId:~p~n Members:~p~n", [LeaderId, NewMemberId, Members]),
  IsGroupLeader = (LeaderId =:= NewMemberId),
  ...
  ?LOG(info, "elected=~p~n", [IsGroupLeader]),
  {ok, NewState}.
...
sync_group(#state{groupId = GroupId, generationId = GenerationId, memberId = MemberId,
  ...
  Request = wolff_kafka_request:sync_group(Conn, KafkaParams),
  %% send sync group request and wait for response
  Response = send_sync(Conn, Request),
  ?LOG(info, "sync_group...~n Response:~p~n", [Response]),
  %% get my partition assignments
  Assignment = kf(member_assignment, Response),
  %% [{topic(), partition(), offset()}]
  TopicAssignments = get_topic_assignments(State, Assignment),
  ?LOG(info, "sync_group...~n assignments received:~p~n", [TopicAssignments]),
  %% 回调分配分区
  ok = MemberModule:assignments_received(MemberPid, MemberId, GenerationId, TopicAssignments),

  NewState = State#state{is_in_group = true},
  start_offset_commit_timer(NewState).

do_stabilize([], _RetryFun, State) -> {ok, State};
...
-spec handle_ack(state(), wolff:group_generation_id(), wolff:topic(),
...
do_commit_offsets_(#state{acked_offsets = []} = State) -> {ok, State};
do_commit_offsets_(#state{offset_commit_policy = consumer_managed} = State) -> {ok, State};
do_commit_offsets_(#state{groupId = GroupId,
  ...
  Retention =
    case is_default_offset_retention(OffsetRetention) of
      true -> ?OFFSET_RETENTION_DEFAULT;
      false -> timer:seconds(OffsetRetention)
    end,

  Request = wolff_kafka_request:offset_commit(Connection, [
    {group_id, GroupId},
    {generation_id, GenerationId},
    {member_id, MemberId},
    {retention_time, Retention},
    {topics, NewTopicOffsets}
  ]),
...
-spec merge_acked_offsets(Offsets, Offsets) -> Offsets when
  Offsets :: [{{wolff:topic(), wolff:partition()}, wolff:offset()}].
merge_acked_offsets(AckedOffsets, OffsetsToAck) ->
  lists:ukeymerge(1, OffsetsToAck, AckedOffsets).

%% Make metadata to be committed together with offsets.
...
-spec bin(iolist()) -> binary().
bin(X) -> list_to_binary(X).

-spec coordinator_id() -> binary().
coordinator_id() -> bin(io_lib:format("~p/~p", [node(), self()])).
...
is_already_connected(#state{connection = Conn}, {Host, Port}) ->
  ...
  iolist_to_binary(Host0) =:= iolist_to_binary(Host) andalso Port0 =:= Port.
...
%% client/script. e.g. wolff@localhost/<0.45.0>_/172.18.0.1
...
translate_members(Members) ->
  lists:map(
    fun(Member) ->
      MemberId = kf(member_id, Member),
      Meta = kf(member_metadata, Member),
      Version = kf(version, Meta),
      Topics = kf(topics, Meta),
      UserData = kf(user_data, Meta),
      {MemberId, #kafka_group_member_metadata{
        version = Version,
        topics = Topics,
        user_data = UserData
      }}
...
assign_partitions(#state{client_id = ClientId, members = Members,
  partition_assignment_strategy = Strategy, member_pid = MemberPid,
  member_module = MemberModule} = State) when ?IS_LEADER(State) ->
  AllTopics = all_topics(Members),
  AllPartitions = [
    {Topic, Partition} || Topic <- AllTopics, Partition <- get_partitions(ClientId, Topic)
  ],
  Assignments = case Strategy =:= callback_implemented of
                  true ->
                    %% 回调模块分配分区
  ...
  lists:map(
    %% Topics_ : [{topic, [Partition1, Partition2,...,PartitionN]}]
    fun({MemberId, Topics_}) ->
      PartitionAssignments = lists:map(
...
assign_partitions(#state{} = _State) -> [].

%% collect topics from all members
-spec all_topics([member()]) -> [wolff:topic()].
all_topics(Members) ->
  lists:usort(
    lists:append(
      lists:map(
...
-spec get_partitions(wolff:client_id(), wolff:topic()) -> [wolff:partition()].
get_partitions(ClientId, Topic) ->
  %% 获取每个Topic分区数量
  Count = ?ESCALATE(wolff_client:get_partitions_count(ClientId, Topic)),
  lists:seq(0, Count - 1).

-spec do_assign_partitions(roundrobin_v2, [member()], [{wolff:topic(), wolff:partition()}])
...
roundrobin_assign_loop(TopicPartitions, [], AssignedMembers) ->
  %% all members have received assignments, continue the next round
  roundrobin_assign_loop(TopicPartitions, lists:reverse(AssignedMembers), []);
roundrobin_assign_loop([{Topic, Partition} | Rest] = TopicPartitions,
    [{MemberId, IsValidAssignment, AssignedTopics} = Member | PendingMembers], AssignedMembers) ->
  case IsValidAssignment(Topic, Partition) of
    true ->
      NewTopics = orddict:append_list(Topic, [Partition], AssignedTopics),
      NewMember = {MemberId, IsValidAssignment, NewTopics},
      roundrobin_assign_loop(Rest, PendingMembers, [NewMember | AssignedMembers]);
    false ->
      %% The fist member in the pending list is not interested in this
      %% topic-partition,
      roundrobin_assign_loop(TopicPartitions, PendingMembers, [Member | AssignedMembers])
  end.

%% Extract the partition assignments from SyncGroupResponse
%% then fetch the committed offsets of each partition.
-spec get_topic_assignments(state(), binary() | [kpro:struct()]) -> wolff:received_assignments().
...
get_topic_assignments(#state{} = State, Assignment) ->
  PartitionAssignments = kf(topic_partitions, Assignment),
  TopicPartitions0 = lists:map(
    fun(PartitionAssignment) ->
      Topic = kf(topic, PartitionAssignment),
      Partitions = kf(partitions, PartitionAssignment),
...
-spec get_committed_offsets(state(), [{wolff:topic(), wolff:partition()}]) ->
  [{{wolff:topic(), wolff:partition()}, wolff:offset()}].
get_committed_offsets(#state{offset_commit_policy = consumer_managed,
  member_pid = MemberPid, member_module = MemberModule}, TopicPartitions) ->
...
get_committed_offsets(#state{offset_commit_policy = commit_to_kafka_v2,
  groupId = GroupId, connection = Conn}, TopicPartitions) ->
  GroupedPartitions = wolff_utils:group_per_key(TopicPartitions),
  Request = wolff_kafka_request:offset_fetch(Conn, GroupId, GroupedPartitions),
  Response = send_sync(Conn, Request),
  ?LOG(info, "get_committed_offsets... Response:~p~n", [Response]),
  ?ESCALATE_EC(kf(error_code, Response, ?no_error)),
  TopicOffsets = kf(responses, Response),
  CommittedOffsets = lists:map(
    fun(TopicOffset) ->
      Topic = kf(topic, TopicOffset),
      PartitionOffsets = kf(partition_responses, TopicOffset),
      lists:foldl(
        fun(PartitionOffset, Acc) ->
          Partition = kf(partition, PartitionOffset),
          Offset0 = kf(offset, PartitionOffset),
          Metadata = kf(metadata, PartitionOffset),
          ?ESCALATE_EC(kf(error_code, PartitionOffset)),
  ...
  lists:append(CommittedOffsets).

-spec resolve_begin_offsets([TP], [{TP, wolff:offset()}], boolean()) ->
  wolff:received_assignments() when TP :: {wolff:topic(), wolff:partition()}.
resolve_begin_offsets([], _CommittedOffsets, _IsConsumerManaged) -> [];
resolve_begin_offsets([{Topic, Partition} | Rest], CommittedOffsets, IsConsumerManaged) ->
  ?LOG(info, "resolve_begin_offsets... Topic:~p Partition:~p CommittedOffsets:~p~n", [Topic, Partition, CommittedOffsets]),
  BeginOffset = case lists:keyfind({Topic, Partition}, 1, CommittedOffsets) of
  ...
  Assignment = #wolff_received_assignment{
    topic = Topic,
    partition = Partition,
    begin_offset = BeginOffset
  },
  [Assignment | resolve_begin_offsets(Rest, CommittedOffsets, IsConsumerManaged)].

%% Upgrade offset from old roundrobin protocol to new.
%% old (before roundrobin_v2) wolff commits acked offsets not begin_offset
-spec maybe_upgrade_from_roundrobin_v1(wolff:offset(), binary()) -> wolff:offset().
...
%% Here we try to recognize wolff commits using a regexp,
...
maybe_send_heartbeat(#state{is_in_group = true, groupId = GroupId,
  memberId = MemberId, generationId = GenerationId, connection = Conn} = State) ->
  Params = [
    {group_id, GroupId},
    {generation_id, GenerationId},
    {member_id, MemberId}
  ],
  Request = kpro:make_request(heartbeat, 0, Params),
  ok = kpro:request_async(Conn, Request),
  NewState = State#state{hb_ref = {Request#kpro_req.ref, os:timestamp()}},
  {ok, NewState};
maybe_send_heartbeat(#state{} = State) ->
  %% do not send heartbeat when not in group
  {ok, State#state{hb_ref = ?undef}}.

send_sync(Conn, Request) ->
  send_sync(Conn, Request, 5000).

send_sync(Conn, Request, Timeout) ->
  ?ESCALATE(wolff_utils:request_sync(Conn, Request, Timeout)).
...
assert_commit_response(Response) ->
  ErrorSet = collect_commit_response_error_codes(Response),
  case gb_sets:to_list(ErrorSet) of
    [] -> ok;
    [EC] -> ?ESCALATE_EC(EC);
    _ -> erlang:error({commit_offset_failed, Response})
  end.

-spec collect_commit_response_error_codes([kpro:struct()]) -> gb_sets:set().
collect_commit_response_error_codes(Response) ->
  lists:foldl(
    fun(Rsp, Acc1) ->
      Partitions = kpro:find(partition_responses, Rsp),
      lists:foldl(
        fun(Partition, Acc2) ->
...
start_offset_commit_timer(#state{offset_commit_timer = Old,
  offset_commit_policy = Policy, offset_commit_interval_seconds = Seconds} = State) ->
  case Policy of
    consumer_managed ->
      {ok, State};
    commit_to_kafka_v2 ->
      is_reference(Old) andalso erlang:cancel_timer(Old),
